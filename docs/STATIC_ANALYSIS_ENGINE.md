...
The Static Analysis Engine (SAE) is a deterministic subsystem that
measures structural properties of a candidate artifact and compares them
to the local budget produced by the Budget Compiler.
...
The SAE MUST read declared_state_count and declared_transition_count
directly from the validated contract.
...
Definition:
  The number of non-blank, non-comment lines between the opening brace
  of a function and its matching closing brace.
...
Definition:
  The number of unique direct module dependencies imported or required
  by the artifact.
...
Violation:
  ...
  If transition_count >
       local_budget.max_states * local_budget.max_transitions_per_state → FAIL.
...
function run_static_analysis(source, local_budget, contract):
    ...
    if metrics.transition_count >
       local_budget.max_states * local_budget.max_transitions_per_state:
        fail(SAE_E_TRANSITION_COUNT)
