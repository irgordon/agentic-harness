...
The Ledger is the authoritative, append-only event log for a Harness run.
It records all deterministic gate outcomes, attempt-level results,
exemption applications, and final run status.
...

{
  ...
  "payload": { ... }   // event-specific, deterministic
}
...
The attempt number is carried in the envelope (`attempt` field), NOT in
payloads.
