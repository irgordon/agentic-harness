...
This system transforms a validated Contract into a Generated Artifact
through a deterministic validation and freeze pipeline. The V1 system
guarantees:
...
The generator (LLM/agent) is explicitly OUTSIDE the trusted core unless
fully pinned and reproducible. The trusted core validates artifacts; it
does not guarantee end‑to‑end generation determinism.
...
Absent these, the system guarantees deterministic validation of a
candidate artifact, not deterministic generation.
...
The contract expresses only what the Budget Compiler and Static Analysis
Engine can enforce deterministically.
